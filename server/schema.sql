-- Banqi federated relay: schema. SQLite, single file.
-- Applied idempotently from db.mjs.
...
-- finalize_claims is used to collect game-over reports from both clients. When
-- both rows for a game agree, the server applies Elo and marks the game
-- complete. If they disagree, the game is marked 'disputed' with no rating
-- change.
